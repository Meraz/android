package com.example.app_android.util;

public abstract class MapPlaceIdentifiers {

	//Toggle IDs
	public static final int TOGGLE_ID_NO_TOGGLE			= -1;
	public static final int TOGGLE_ID_CAMPUS_HOUSES		= 0;
	public static final int TOGGLE_ID_STUDENT_UNION		= 1;
	public static final int TOGGLE_ID_STUDENT_PUBS		= 2;
	
	//Marker icon IDs
	public static final int MARKER_ICON_ID_DEBUG		= -1;
	public static final int MARKER_ICON_ID_HOUSE_A 		= 0;
	public static final int MARKER_ICON_ID_HOUSE_B 		= 1;
	public static final int MARKER_ICON_ID_HOUSE_C 		= 2;
	public static final int MARKER_ICON_ID_HOUSE_D 		= 3;
	public static final int MARKER_ICON_ID_HOUSE_G 		= 4;
	public static final int MARKER_ICON_ID_HOUSE_H 		= 5;
	public static final int MARKER_ICON_ID_HOUSE_J 		= 6;
	public static final int MARKER_ICON_ID_ROTUNDAN 	= 7;
	public static final int MARKER_ÌCON_ID_BSK_OFFICE 	= 8;
}